...
class SearchAdapter(val callback : SearchScreenCallback) : ListAdapter<LocalModel,SearchViewHolder>(DiffUtilClass<LocalModel>()) {
    ...
    override fun onBindViewHolder(holder: SearchViewHolder, position: Int) = holder.present(getItem(position))
}